...
void SevSeg::Begin(boolean mode_in, byte numOfDigits,
	byte dig1, byte dig2, byte dig3, byte dig4, byte dig5,
...
void SevSeg::Begin(boolean mode_in, byte numOfDigits,
	...
	byte segDP)
{
  Begin(mode_in, numOfDigits, dig1, dig2, dig3, dig4, dig5, 255, 255, segA, segB, segC,
...
void SevSeg::DisplayString(char* toDisplay, byte DecAposColon)
{
	...
	for(byte digit = 1 ; digit < (numberOfDigits+1) ; digit++)
	{
      ...
      case 5:
  			...
  			break;
			//This only currently works for 5 digits
		}
		...
		if ((DecAposColon & (1<<(digit-1))) && (digit < 6)) //Test DecAposColon to see if we need to turn on a decimal point
